...
export default function PeminjamanPage() {
  ...
  const fetchPeminjaman = async () => {
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error("Server mengalami masalah");
      }
  ...
  const handleStatusUpdate = async (id: number, status: string) => {
    try {
      ...
      if (response.ok) {
        toast.success(
          `Peminjaman berhasil ${status === "approved" ? "disetujui" : "ditolak"}`,
        );
        fetchPeminjaman();
      } else {
        toast.error(
          `Gagal ${status === "approved" ? "menyetujui" : "menolak"} peminjaman`,
        );
      }
    } catch (error) {
      toast.error(
  ...
  const menungguPersetujuan = peminjaman.filter(
  ...
  const currentItems = filteredPeminjaman.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );
    ...
    <div className="min-h-screen bg-gradient-to-r from-blue-100 via-purple-100 to-pink-100 flex">
      ...
      <div className="flex-1 p-4 md:p-6">
        <h1 className="text-xl md:text-2xl font-bold mb-6">
          Manajemen Peminjaman Kendaraan
        </h1>
        {/* Statistics Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-white rounded-lg shadow p-4 flex justify-between items-center">
            <div>
              <h3 className="text-xs md:text-sm text-gray-500 font-medium">
                Total Peminjaman
              </h3>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm mt-1 flex items-center gap-1.5">
                          ...
                          {item.driver || "Membawa Sendiri"}
                        </div>
                        <div className="text-sm text-gray-600 mt-1">
                          {item.purpose || "Tidak ada keterangan"}
                        </div>
                      </div>
                    </div>
